...
import { dateTableFormat, getStatusFromMulta, minutFormat, moneyFormat } from "../../utils/formats";
import { MultaStatusBadge } from "../MultaStatus/MultaStatus";
...
const MultaLine: React.FC<MultaLineProps> = ({ multa, onClick, type }) => {
    ...
    return (
        <div onClick={() => onClick(multa)} className={`multa-line`}>
            ...
            {type === MultaType.update && <div className="multa-line__cost">{multa && multa.player.shortName}</div>}
            {type === MultaType.resume && <div className="multa-line__date">{multa && dateTableFormat(multa.date)}</div>}
            {type === MultaType.resume && <div className="multa-line__mins-late">{minutFormat(multa?.minsLate)}</div>}
            {type === MultaType.update && <div className="multa-line__status">{multa && <MultaStatusBadge status={getStatusFromMulta(multa)} />}</div>}
